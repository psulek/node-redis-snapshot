...
function readBuffer(buffer) {
	...
	function availData(cnt) {
		var avail = (pos + cnt) < bufferLength;
		bufferEof = !avail;
		return avail;
	}
	...
	if (!success) errorCode = readBufferError.IncompleteBuffer;
		...
		var header_signature = bufferXtra.sliceNew(buffer, pos, BYTES_HEADER_SIGNATURE);
		...
		if (availData(BYTES_UINT32)) {
			...
			// reqBytes => type + ttlFormat
			var reqBytes = BYTES_BYTE + BYTES_BYTE;
			...
			for (var i = 0; i < keyCount; i++) {
				if (availData(reqBytes)) {
					...
					var ttl = 0;
					if (ttlFormat !== structure.TTLFormats.None && availData(BYTES_UINT32)) {
						ttl = buffer.readUInt32LE(pos);
						pos += BYTES_UINT32;
					}

					var keyLength = 0;
					if (availData(BYTES_UINT32)) {
						keyLength = buffer.readUInt32LE(pos);
						pos += BYTES_UINT32;
					}
					...
					if (keyLength > 0 && availData(keyLength)) {
						...
						key = bufferXtra.sliceNew(buffer, pos, keyLength);
						pos += keyLength;
					}
					...
					if (availData(BYTES_INT32)) {
						...
						if (crcLength > 0 && availData(crcLength)) {
							...
							crc = bufferXtra.sliceNew(buffer, pos, crcLength);
							pos += crcLength;
						}
					}

					var item = structure.createRedisItem(key, keyType, ttl, ttlFormat,
				...
				for (var i = 0; i < keyCount; i++) {
					if (availData(BYTES_BYTE)) {
						...
						if (dataLength > 0 && availData(dataLength)) {
							...
							data = bufferXtra.sliceNew(buffer, pos, dataLength);
							pos += dataLength;
						}
